#include "active_3d_planning_core/module/trajectory_evaluator/shannon_mutual_information_gain.h"
...
ModuleFactoryRegistry::Registration<ShannonMI>
        ShannonMI::registration("ShannonMutualInformation");

    ShannonMI::ShannonMI(PlannerI& planner)
    : FrontierEvaluator(planner) {}

void ShannonMI::setupFromParamMap(Module::ParamMap* param_map) {
  ...
  if (!map_) {
    planner_.printError(
        "'ShannonMutualInformation' requires a map of type 'TSDFMap'!");
  }
...
bool ShannonMI::storeTrajectoryInformation(
...
bool ShannonMI::computeGainFromVisibleVoxels(
    TrajectorySegment* traj_in) {
  traj_in->gain = 0.0;
  if (!traj_in->info) {
    return false;
...
double ShannonMI::getVoxelValue(const Eigen::Vector3d& voxel,
  ...
  if (voxel_state == map::TSDFMap::OCCUPIED) {
    ...
    if (abs(0.5 - gain) < (0.5 - p_min_impact_factor_)) {
      return -(gain*log2(gain) + (1.0 - gain) * log2(1.0 - gain));
    }
...
void ShannonMI::visualizeTrajectoryValue(
